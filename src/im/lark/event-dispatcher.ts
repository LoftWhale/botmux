...
export function updateBotOpenIdCrossRef(
  ...
  if (entries.length > 0) {
    try {
      // Locked read-modify-write (bot-openid-crossref-store): concurrent daemons
      // merging different peers must not clobber each other's entries. The
      // peer-cross-ref-store read cache revalidates on the new file fingerprint.
...
export function evaluateTalk(
  ...
  if (sessionGroup) {
    ...
    // 便于 owner 后续 /grant @x N 续杯/重置）。goal 群即使带 oncall 绑定也不走
    // oncall 放行：goal 会话有自己的成员/配额语义（isGoalChat 损坏时失败开放）。
    return oncallTalk(bot, larkAppId, chatId, senderOpenId);
  }

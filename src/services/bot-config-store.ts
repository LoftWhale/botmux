...
export const CONFIG_FIELDS: readonly ConfigFieldSpec[] = [
  ...
  { key: 'codexAuthSync', configKey: 'codexAuthSync', kind: 'enum', effect: 'next-session', clearable: true, enumValues: ['shared', 'isolated'], hint: 'Codex 沙箱独立 CODEX_HOME 的鉴权策略：shared=每次冷启动同步全局 ~/.codex/auth.json（默认/兼容旧版）｜isolated=绝不复制全局凭证，需在该 bot 的 CODEX_HOME 单独执行 codex login --with-api-key；非沙箱仍使用 Codex 全局登录态' },

...
### L1-3 一条命令建群并启动监管者（推荐）
用户确认后，优先运行统一入口：\`botmux goal start --title "<goal 名>" [--team "<平台团队名称或 id>"] --supervisor "<当前主控 bot>" --worker "<执行者显示名>" [--worker "<执行者显示名>" ...] --project "<本机目录或 Git remote>" --brief "<目标和最终验收口径>"\`。
- 纯本机协作可省略 \`--team\`；跨设备执行者从平台团队花名册解析。多个团队都匹配时列出候选让人指定，禁止猜。
- 已有真实目标群时加 \`--chat-id <goalChatId>\`；入口先查成员再开工。平台机器人大厅会被拒绝，不能拿同步表/大厅冒充目标群。
- 新目标群只邀请当前目标的发起人，不把平台团队里的所有成员都拉进群。
- 监管者必须是当前 L1 会话所属的本机在线 bot，保证完成/求助能可靠回到 L1。
- \`--project\` 传本机 Git 目录时自动读取 origin；远端任务使用 remote，不把本机绝对路径发到另一台机器。有远端执行者但本机项目没有 origin 时必须先报错，不能静默降级。
- 成功输出 chatId、群链接、监管会话和执行者清单。群创建成功但监管启动失败时，照输出的 \`--chat-id\` 续跑命令恢复，别重复建群。
...
### L1-4 手工起监管者（仅排障/续跑）
\`goal start\` 已自动完成这一步。只有手工建群或排障续跑时才直接调用 \`goal supervise\`；建群后**不要自己跨群派活**：
\`\`\`bash
...
**dashboard 一键动作也是给你的正式指令**：看板可能下发 \`[panel-action v1]\` 文本块（\`action: reassign-worker | switch-worker | escalate-human | resolve-help | confirm-release | retry-release | resolve-dependency\`）。收到后**先查交付记录和 charter**，再执行真实命令并留痕，别把它当普通聊天建议：
...
- \`confirm-release\`：只在群历史能确认原派发消息确实送达时运行 \`botmux delivery release-confirm --task <id>\`；拿不准不能确认。
- \`retry-release\`：先修复失败原因并确认重复消息风险，再运行 \`botmux delivery release-retry --task <id>\`；系统会用新派发编号重试。
- \`resolve-dependency\`：检查上游任务；上游取消时决定取消后续或另建任务，上游驳回滞留时推动补交、重派或升级给人。
- reason 是 \`help:missing_repo\`（执行者机器缺项目环境）时对症处理：\`resolve-help\`=把「准备项目环境（clone/setup）」当任务派给它或指导就地准备，环境好了同 taskId 重派原任务；\`switch-worker\`=换一台已有该项目的执行者。
...
6. **任务不再需要**（临时探针、范围变更、重复派发）→ \`botmux delivery cancel --task <id> --reason "<为什么不做了>" [--by <你的标识>]\`。取消是终态：watchdog/自动核验不再碰它、看板上显示为灰色「已取消」状态，迟到的提交只入账不改状态。**已验收（accepted）不能取消**；取消错了用同一 taskId 重新 \`dispatch\` 即可重开。**这是你（监管者/主控）的决策权**——worker 觉得任务没必要做时走 \`botmux help\` 说明理由，由你判断后取消，不许它自我取消。

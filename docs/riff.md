...
- config 系（kaboo-cli / aiden-cli / cjadk）按**当前平台**解析 config 根:Linux 用 `$XDG_CONFIG_HOME`（未设则 `~/.config`）、macOS 用 `~/Library/Application Support`——只探测当前平台的那一个,不会跨平台混用;**bytedcli 不读 `$XDG_DATA_HOME`**（固定 `~/.local/share`,已实测确认）

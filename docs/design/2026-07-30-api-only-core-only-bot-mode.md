...
**8. API-task Full Access 例外** — 上述 no-transport 文件隔离仍是默认值。只有 owner 在 bot 配置中显式设置 `apiTaskFullAccess: true` 后创建的 HTTP virtual/API-task session，才会冻结该能力并跳过默认文件隔离；`/api/trigger` 请求不能携带或补授该能力，普通聊天与旧 session 不受影响。bot 的 `readIsolation`、session 已冻结的 `sandbox` 与 host `BOTMUX_SANDBOX=1` 始终优先。启用意味着该 session 的 agent 进程拥有同一 OS 用户可见的 host 文件与 IPC 能力（含磁盘上 `bots.json` 内各 bot 的 app secret 等原本由文件沙盒遮蔽的凭证文件；env 级删除本 bot `BOTMUX_LARK_APP_SECRET` 只关闭 Botmux 自身发送链路，不构成恶意代码下的凭证硬隔离）；这是为必须使用 host IPC 的受信 bot 明确放弃文件隔离，而不是路径 carve-out。device-credential 强制隔离不在放弃范围内，仍独立生效。

...
describe('/rename production routing — must not pre-create a session (review P1)', () => {
  ...
  it('thread safety-net: passes the accepted reply id into the first worker', async () => {
    ...
    expect(mocks.forkWorker).toHaveBeenCalledTimes(1);
    // master 侧 forkReservedInitialSession 现在把可信调用方身份随 turnId 一起
    // 传给首个 worker（{ turnId, trustedCaller }），safety-net 断言同步跟进。
    expect(mocks.forkWorker.mock.calls[0]?.[2]).toEqual({
      turnId: 'om_workflow_reply',
      trustedCaller: { requestLarkAppId: APP, requestUserOpenId: OWNER },
    });
  });

...
固定话题的任务会一直投到创建它的那个话题里；如果群里的讨论已经搬到了别的话题，提醒就落在没人看的地方。`--follow-active` 让任务在**每次触发时**重新选目标：投到本群里**人最近说话**的那个话题。
...
- **只按真人消息判「活跃」**，bot 自己的回复和定时任务的输出不算——否则一个每 30 分钟开火的任务会把自己所在的话题一直刷成「最活跃」，从此跟着自己走。
- **跨 bot 判定**：人在哪个话题，是人的属性，不是某个 bot 的属性。群里几个 bot 各有各的会话记录，判定时全部一起看。
- **找不到就沿用上次落点，从不新建话题**：没有任何话题有过真人发言（比如刚重启、记录还没建起来）时，投到上一次落过的话题；第一次触发前就是创建时的话题。
